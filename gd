...
usage() {
    ...
    echo "  -o , --bond-order                   Show Wilber Bond Order matrix, input FCHK file"
...
for file in "$@"; do
  ...
  if [ "$zpe" = true ] || [ "$all" = true ]; then
    Energy=$(grep Done $file | tail -n 1 | sed 's/.*=\s*\(-*[0-9.]*\)\s*.*/\1/')
    eV=$(echo "scale=20; $Energy * 27.211324570273" | bc)
    
    #ZPE corrected energy
    ZPE_cor_EE=$(grep ' Sum of electronic and zero-point Energies' $file | sed ' s: Sum of electronic and zero-point Energies=          ::')
  ...
  if [ "$homo_lumo" = true ] || [ "$all" = true ]; then 
    # ALPHA
    read alpha_first_value alpha_last_value < <(tac $file | grep 'eigenvalues --' | grep -m1 -B 1 'Alpha  occ.' | awk 'NR==1 {first_value=$5} NR==2 {last_value=$NF} END {print first_value, last_value}')
    ...
    beta_homo=-10000
    beta_lumo=10000
    if grep -q "Beta  occ." $file; then
    	read beta_first_value beta_last_value < <(tac $file | grep 'eigenvalues --' | grep -m1 -B 1 'Beta  occ.' | awk 'NR==1 {first_value=$5} NR==2 {last_value=$NF} END {print first_value, last_value}')
  ...
  if [ "$dipole_moment" = true ] || [ "$all" = true ]; then 
    dm=$(printf "%10.10f" $(tac $file | grep -m 1 "Electric dipole moment" --before-context 3 | head -n 1 | awk '{print $3}' | sed 's/D/e/'))
  ...
  if [ "$polarizability" = true ] || [ "$all" = true ]; then 
    pol=$(printf "%10.10f" $(tac $file | grep -m 1 " iso" | awk '{print $2}'| sed 's/D/e/'))
  ...
  if [ "$hyperpolarizability" = true ] || [ "$all" = true ]; then 
    ...
    vals=()
    for i in "${arr[@]}"; do
      result=$(printf "%10.10f" $(tac "$file" | grep -m 1 "$i" | awk '{print $2}'| sed 's/D/e/'))
        vals+=("$result")
    done
  ...
  if [ "$point_group" = true ] || [ "$all" = true ]; then
    pg=$(tac $file | grep -m 1 "point group" | awk '{print $4}')
  ...
  if [ "$sume_gas_charge" = true ]; then #  || [ "$all" = true ]
    mul=$(tac $file | grep -E "Mulliken .* hydrogens" -m 1 -A $(echo "$sume_gas_charge_value + 1" | bc) | tail -n $sume_gas_charge_value | awk '{s+=$3} END {print s}')
  
    nbo=$(tac $file | grep "Natural Population" -m 1 -A $(echo "$sume_gas_charge_value" + 3 | bc) | tail -n $sume_gas_charge_value | awk '{s+=$3} END {print s}')
    ...
    if [ "$title" = true ]; then
      ...
      tac $file | grep -E "Mulliken .* hydrogens" -m 1 -A $(echo "$sume_gas_charge_value + 1" | bc) | tail -n $sume_gas_charge_value | awk '{ printf "%s", $2 }'
      echo
      printf " = ${GREEN} $mul ${NC} \n\n"
      echo "Sum of NBO charges of:"
      tac $file | grep "Natural Population" -m 1 -A $(echo "$sume_gas_charge_value" + 3 | bc) | tail -n $sume_gas_charge_value | awk '{ printf "%s", $1 }'
      echo
      printf " = ${GREEN} $nbo ${NC} \n\n"
    fi
  fi
  ...
  if [ "$bond_length" = true ]; then 
    awk '/Distance matrix/, /Stoichiometry/' "$file" | tac | sed '/Distance matrix/q' | tac
  fi
